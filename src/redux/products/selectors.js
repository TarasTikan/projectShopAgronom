...
export const selectFilteredProducts = createSelector([
  ...
  selectProducts,
      ...
      const normalizator = filterCulture.map(item => item.toUpperCase());
const noFilter = filterProducer.length === 0 && filterCulture.length === 0 && filterNumber.length === 0;
if(noFilter) {
    return products
}
    ...
    return products.filter(({ culture }) =>
      normalizator.every(crop => culture.toUpperCase().includes(crop))
    );
  } else {
